    ...
    <>
      ...
      <Container maxWidth="xl" component="main">
        <Routes>
          ...
          <Route path="/track-history" element={<ProtectedRoute isAllowed={
            user && (user.role === 'admin' || user.role === 'user')}>
            <TrackHistory />
          </ProtectedRoute>}
          />
          <Route path="/track/new" element={<ProtectedRoute isAllowed={
            user && (user.role === 'admin' || user.role === 'user')}>
            <NewTrack />
          </ProtectedRoute>}
          />
          <Route path="/album/new" element={<ProtectedRoute isAllowed={
            user && (user.role === 'admin' || user.role === 'user')}>
            <NewAlbum />
          </ProtectedRoute>}
          />
          <Route path="/artist/new" element={<ProtectedRoute isAllowed={
            user && (user.role === 'admin' || user.role === 'user')}>
            <NewArtist />
          </ProtectedRoute>}
          />
