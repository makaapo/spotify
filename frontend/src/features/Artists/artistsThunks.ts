...
export const getArtists = createAsyncThunk<Artist[]>('artists/get-all', async () => {
  const { data: artist } = await axiosApi.get<Artist[]>('/artists');
  return artist;
});

export const getArtistById = createAsyncThunk<Artist, string>('artists/get-by-id', async (artistId) => {
...
export const publishArtist = createAsyncThunk<void, string>('artists/publish', async (id) => {
