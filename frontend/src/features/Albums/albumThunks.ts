...
export const getAlbumsByArtist = createAsyncThunk<Album[], string>('albums/get-by-artist', async (artistId) => {
  const { data: albums } = await axiosApi.get<Album[]>(`/albums?artist=${artistId}`);
  return albums;
});

export const createAlbum = createAsyncThunk<void, AlbumMutation>('artists/create', async (albumMutation) => {
...
export const publishAlbum = createAsyncThunk<void, string>('albums/publish', async (id) => {
